...
def isDrillableCylinder(obj, candidate, tooldiameter=None, vector=App.Vector(0, 0, 1)):
    ...
    def raisedFeature(obj, candidate):
        ...
        startLidCenter = App.Vector(candidate.BoundBox.Center.x,
                                    candidate.BoundBox.Center.y,
                                    candidate.BoundBox.ZMax)

        endLidCenter = App.Vector(candidate.BoundBox.Center.x,
                                  candidate.BoundBox.Center.y,
                                  candidate.BoundBox.ZMin)

        return obj.isInside(startLidCenter, 1e-6, False) \
            or obj.isInside(endLidCenter, 1e-6, False)
...
def isDrillableCircle(obj, candidate, tooldiameter=None, vector=App.Vector(0, 0, 1)):
    ...
    if candidate.ShapeType == 'Face':
        ...
        if len(candidate.Edges) == 1 \
           and type(candidate.Edges[0].Curve) == Part.Circle:  # Regular circular face
            edge = candidate.Edges[0]
        elif len(candidate.Edges) == 2 \
            and type(candidate.Edges[0].Curve) == Part.Circle \
            and type(candidate.Edges[1].Curve) == Part.Circle:  # process a donut
            e1 = candidate.Edges[0]
            e2 = candidate.Edges[1]
            edge = e1 if e1.Curve.Radius < e2.Curve.Radius else e2
        else:
            PathLog.debug('expected a Face with one or two circular edges got a face with {} edges'.format(len(candidate.Edges)))
            return False
...
def isDrillable(obj, candidate, tooldiameter=None, vector=App.Vector(0, 0, 1)):
    ...
    """
    PathLog.debug('obj: {} candidate: {} tooldiameter {} vector {}'.format(obj, candidate, tooldiameter, vector))
    ...
    try:
        if candidate.ShapeType == 'Face' and str(candidate.Surface) == '<Cylinder object>':
...
def compareVecs(vec1, vec2):
    ...
    return numpy.isclose(angle, 0, rtol=1e-05, atol=1e-06) or \
        numpy.isclose(angle, 180, rtol=1e-05, atol=1e-06)
